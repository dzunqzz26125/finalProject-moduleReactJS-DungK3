...
const Workspaces = () => {
  ...
  const fetchAll = async () => {
    const [wsRes, boardsRes, membersRes, usersRes] = await Promise.all([
      api.get("/workspaces"),
      api.get("/boards"),
  ...
  const getUserRole = (workspaceId) => {
    const m = members.find((m) => m.workspaceId === workspaceId && m.userId === currentUser.id);
    return m?.role || null;
  };
  ...
  return (
    <div className="ml-64 pt-20 p-8 min-h-screen transition-colors" style={{ backgroundColor: "var(--bg-base)" }}>
      <header className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-black uppercase tracking-tighter font-['Space_Grotesk']" style={{ color: "var(--accent)" }}>Workspaces</h1>
          <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>{workspaces.length} workspace(s)</p>
        </div>
        <button
      ...
      <div className="space-y-8">
        {workspaces.map((ws) => {
          ...
          return (
            <div key={ws.id} className="border rounded-xl overflow-hidden transition-colors" style={{ backgroundColor: "var(--bg-elevated)", borderColor: "var(--border)" }}>
              <div className="flex items-center gap-4 p-5 border-b" style={{ borderColor: "var(--border)" }}>
                <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white text-lg font-black" style={{ backgroundColor: ws.color }}>
                  {ws.name[0].toUpperCase()}
                </div>
                <div className="flex-1">
                  <h3 className="font-bold font-['Space_Grotesk']" style={{ color: "var(--text-primary)" }}>{ws.name}</h3>
                  <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                    {wsBoards.length} boards • {wsMembers.length} members • <span className="uppercase" style={{ color: "var(--accent)" }}>{role}</span>
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setExpandedWs((p) => ({ ...p, [ws.id]: !p[ws.id] }))} className="p-2 transition-colors" style={{ color: "var(--text-muted)" }}>
                      ...
                      {wsMembers.map((m) => {
                        ...
                        return (
                          <div key={m.id} className="flex items-center gap-2 px-3 py-1.5 rounded-full" style={{ backgroundColor: "var(--bg-surface)" }}>
                            ...
                            <span className={`text-[9px] uppercase font-bold ${m.role === "admin" ? "text-yellow-400" : m.role === "viewer" ? "text-gray-400" : "text-blue-400"}`}>{m.role}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {(role === "admin" || role === "member") && (
                      <button
                        ...
                        className="h-24 rounded-xl flex flex-col items-center justify-center border-2 border-dashed transition-all group"
                        style={{ backgroundColor: "var(--bg-surface)", borderColor: "var(--border)" }}
                      >
                        <span className="material-symbols-outlined transition-colors" style={{ color: "var(--text-faint)" }}>add</span>
                        <span className="text-xs font-bold uppercase tracking-wider transition-colors mt-1" style={{ color: "var(--text-faint)" }}>New Board</span>
                      </button>
                    )}
                    {wsBoards.map((board) => (
                      <div
