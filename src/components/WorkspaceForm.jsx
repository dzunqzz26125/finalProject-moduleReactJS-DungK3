...
const COLORS = ["#0079bf", "#4bbf6b", "#89609e", "#cd5a91", "#e6a817", "#eb5a46"];

const WorkspaceForm = ({ onClose, workspace }) => {
  const user = JSON.parse(localStorage.getItem("users") || "{}");
  const isEdit = !!workspace;
  const { register, handleSubmit, watch, formState: { errors } } = useForm({
    defaultValues: isEdit ? workspace : { color: COLORS[0], ownerId: user.id },
  });

  const onSubmit = async (data) => {
    if (isEdit) {
      await api.put(`/workspaces/${workspace.id}`, data);
    } else {
      const res = await api.post("/workspaces", { ...data, ownerId: user.id, createdAt: new Date().toISOString().split("T")[0] });
      await api.post("/workspace_members", { workspaceId: res.data.id, userId: user.id, role: "admin" });
    }
    onClose(true);
  };
      ...
      <div className="relative bg-[#1C1B1B] border-2 border-[#CCFF00] w-full max-w-md shadow-[0_0_50px_rgba(204,255,0,0.1)]">
        <div className="bg-[#CCFF00] px-6 py-4 flex items-center justify-between">
          <h2 className="text-[#131313] font-black text-xl uppercase tracking-tighter font-['Space_Grotesk']">
            {isEdit ? "Edit_Workspace" : "New_Workspace"}
          </h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-5">
          <div>
            <label className="text-[#CCFF00] text-[10px] uppercase tracking-widest block mb-2">Name *</label>
            <input
          ...
          <div>
            <label className="text-[#CCFF00] text-[10px] uppercase tracking-widest block mb-2">Description</label>
            <textarea
              {...register("description")}
              className="w-full bg-[#131313] border-2 border-[#444] focus:border-[#CCFF00] text-[#E5E2E1] p-3 text-sm focus:ring-0 resize-none transition-all"
              rows={2}
              ...
              {COLORS.map((c) => (
                <label key={c} className="cursor-pointer">
                  ...
                  <div
                    ...
                    style={{
                      backgroundColor: c,
                      outline: selectedColor === c ? "3px solid #CCFF00" : "none",
                      outlineOffset: "2px",
                    }}
                  />
                </label>
